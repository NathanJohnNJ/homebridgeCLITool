...
#	echo "		Wardrobe Lightstrip: wardrobe"
	...
	state=$(/Volumes/MAC/coding/scripts/homebridgeCLITool/macVersion/getstate.sh $1)
	if [ $state -eq 1 ]; then
		until [ $state -eq 0 ]; do
			/Volumes/MAC/coding/scripts/homebridgeCLITool/macVersion/off.sh $1
			echo "$1 is now off."
			break
		done
	else
			until [ $state -eq 1 ]; do
			/Volumes/MAC/coding/scripts/homebridgeCLITool/macVersion/on.sh $1
					echo "$1 is now on."
			break
		done
	fi
